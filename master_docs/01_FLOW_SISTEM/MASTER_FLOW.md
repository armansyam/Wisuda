...
> [!IMPORTANT]
> **Prinsip Arsitektur Utama Sistem Studio (v2.1.0):**
...
> 2. **Dual-Payment Gateways**: Fleksibilitas pembayaran via **Transfer Bank Manual** (upload bukti transfer) ATAU **Direct Dynamic QRIS** (iPaymu Gateway dengan auto-verifikasi webhook seketika).
> 3. **Interactive Client Moodboard & Auto Briefing Sheet**: Klien dapat menyusun referensi pose via portal `/moodboard/:token`, dan sistem otomatis menghasilkan **PDF Briefing Sheet** untuk fotografer di lapangan.
> 4. **Isolasi Ketat State Database**: Status state pada Tahap 1 Inquiry (`inquiries`) terisolasi total dan tidak pernah dicampur adukkan ke Tahap 2 Client (`bookings`).
> 5. **Strict Google OAuth 3-Step Wizard & Zero Disk Transit**: Konfigurasi Google Drive terproteksi probe-verification ketat 3 tahap, dan pengunggahan master foto 100% direct-stream resumable ke Google Drive Cloud Studio (Zero Disk VPS).
> 6. **Dual-Root Google Drive Storage**: Pemisahan Root 1 (Storage Client sementara ber-retention) dan Root 2 (Master Portofolio Studio permanen seumur hidup via Cloud-to-Cloud copy).
> 7. **Zero Upload FG & Mobile Portal**: FG tidak dibebankan upload file; seluruh penugasan & briefing diakses via Portal Mobile (`freelance.html`).
> 8. **Token Security Protocol (`TRK-...`)**: Seluruh interaksi portal tracking, moodboard, upload bukti, dan galeri seleksi dilindungi token unik anti-IDOR.
> 9. **Dual-Mode (Web SPA + Headless API Engine)**: Dukungan RESTful API (`/api/v1/*`) dengan otentikasi JWT, API Key (`X-API-Key`), dan multi-origin CORS.
...
| 🎓 **Tahap 1: Inquiry & Booking** | 📄 [TAHAP1_alur_inqury.md](./TAHAP1_alur_inqury.md) | Inquiry 1-pintu, Link Booking Terpadu, Timer 3j, Dual Payment (Manual / QRIS), & **Gate 1 Verifikasi DP**. |
| 👥 **Tahap 2: Client Deal & Shooting** | 📄 [TAHAP2_alur_client.md](./TAHAP2_alur_client.md) | Background Drive Mapping, Moodboard Briefing Sheet PDF, Assign FG, Cron 30m Selesai, & **Gate 2 Pelunasan**. |
| 🎬 **Tahap 3: Post-Produksi & Deliverables** | 📄 [TAHAP3_alur_postproduksi.md](./TAHAP3_alur_postproduksi.md) | Direct Upload Admin, Galeri Seleksi Klien (Touch Swipe), Highlight Portofolio, & **Closing Statement (`completed`)**. |
...
| 📱 **Portal Tracking Klien** | 📄 [ALUR_TRACKING_CLIENT.md](./ALUR_TRACKING_CLIENT.md) | Antarmuka `tracking.html`, Token Security, Form DP/Pelunasan/QRIS, Direct Drive Access, Size Calculator, & Closing Card. |
...
| 📁 **Arsitektur Google Drive** | 📄 [STRUKTUR_FOLDER_DRIVE.md](./STRUKTUR_FOLDER_DRIVE.md) | Dual-Root Architecture (Root 1 Client Storage vs Root 2 Master Portfolio) & 3-Step OAuth Wizard. |
