...
### 3.3. Dual Payment Gateway (Transfer Bank Manual vs Direct Dynamic QRIS)
Client memiliki 2 pilihan metode penyelesaian pembayaran DP:

1. **Metode A: Transfer Bank Manual (Upload Bukti Transfer)**:
   - Client mentransfer manual ke rekening bank studio (BCA/Mandiri/BRI).
   - Client mengunggah file foto bukti transfer via form upload di portal.
   - Status tagihan menjadi `⏳ Menunggu Verifikasi DP` $\rightarrow$ Admin memeriksa keaslian mutasi rekening lalu mengeklik tombol **`🔍 Verifikasi DP`** di Admin Panel.

2. **Metode B: Direct Dynamic QRIS (iPaymu Payment Gateway)**:
   - Client memilih opsi **`⚡ Bayar Instan via QRIS`**.
   - Sistem melakukan request ke API iPaymu dan menampilkan QR Code dinamis langsung di layar client.
   - Client memindai QRIS via aplikasi m-Banking / E-Wallet (GoPay, OVO, Dana, ShopeePay, BCA, dll).
   - Sistem menerima Webhook Callback (`/api/webhook/ipaymu`) dengan verifikasi *digital signature*. Begitu pembayaran sukses, status DP seketika berubah otomatis menjadi **`paid`** (*Instant Gate 1 Clearance* tanpa perlu verifikasi manual admin).

### 3.4. Penyusunan Moodboard & Pose Reference Mandiri oleh Client
Setelah booking terkonfirmasi atau saat menerima link booking, client dapat mengakses portal Moodboard (`/moodboard/:token`):
- **Katalog Vendor**: Memilih referensi foto pose terbaik dari katalog portofolio master studio.
- **Upload Foto Eksternal**: Mengunggah foto referensi pribadi dari Pinterest/Instagram ke subfolder Drive `04_Moodboard/`.
- **Auto Briefing Sheet PDF**: Sistem secara otomatis mengompilasi foto referensi pose tersebut ke dalam dokumen PDF briefing yang siap diunduh oleh Fotografer Freelance di Tahap 2.
