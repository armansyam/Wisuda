...
- **Fotografer Siap**: FG menerima WA notifikasi penugasan dan memantau detail tugas di Portal Freelance (`freelance.html`) (*100% Zero Upload File FG*).
- **Briefing Sheet PDF Otomatis**: FG dapat mengunduh **PDF Briefing Sheet** yang memuat seluruh referensi gaya/pose dari moodboard pilihan klien langsung dari portal mobile.
- **Deteksi Bentrok Jadwal (Conflict Detection)**: Sistem secara otomatis memeriksa jadwal penugasan FG lain pada jam dan tanggal yang sama untuk mencegah terjadinya *double-booking* fotografer.
- **Fleksibilitas Ganti FG (Flexible Re-assign)**: Admin dapat mengganti fotografer sewaktu-waktu jika terjadi keadaan darurat, dan sistem otomatis merekonsiliasi status ketersediaan serta honor FG terkait.
- **Hari H Pemotretan**: Pemotretan berlangsung.
- **Otomasi Sesi Selesai Cron (30 Menit)**: Cron service setiap 30 menit otomatis mengecek $WaktuSelesai = TanggalWisuda + JamMulai + DurasiJam$. Begitu waktu lewat, cron otomatis mengeset `is_session_done = 1` (atau dapat dikonfirmasi instan oleh Admin / FG).

### 3.3. Status 3: Menunggu Pelunasan (`need_balance`)
- **Kondisi DP di Awal**: Sesi foto selesai (`is_session_done = 1`), client menyelesaikan sisa pelunasan via link `tracking.html`.
  - **Opsi Transfer Manual**: Client upload bukti pelunasan $\rightarrow$ Admin verifikasi (`balance_status = 'paid'`).
  - **Opsi Instant QRIS**: Client bayar via QRIS iPaymu $\rightarrow$ Webhook callback otomatis mengeset `balance_status = 'paid'` secara seketika.
...
> 1. **Sesi Foto Selesai**: `is_session_done = 1` (otomatis diset oleh Cron 30 menit atau konfirmasi langsung).
> 2. **Pelunasan Terverifikasi**: `balance_status = 'paid'` (via transfer/QRIS) ATAU `balance_amount = 0` (lunas 100% di awal).
