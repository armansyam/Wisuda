<template>
  <div>
    ...
    <div class="flex border-b border-[#E8D5C8] dark:border-slate-800 mb-6">
      <button @click="activeTab = 'active'" class="px-4 py-2.5 text-xs font-semibold uppercase tracking-wider border-b-2 transition" 
      ...
      <button @click="activeTab = 'applications'" class="px-4 py-2.5 text-xs font-semibold uppercase tracking-wider border-b-2 transition flex items-center gap-2" 
        ...
        📩 Pendaftar Baru
        <span v-if="pendingAppsCount > 0" class="px-2 py-0.5 bg-[#D94A3D] text-white text-[9px] font-bold rounded-full animate-pulse">{{ pendingAppsCount }}</span>
      </button>
    </div>
            ...
            <tbody>
              <tr v-for="item in paginatedFreelancers" :key="item.id" class="border-b border-[#E8D5C8]/40 dark:border-slate-800/60 hover:bg-[#FFF8F3] dark:hover:bg-slate-800/50 text-[#2D1B14] dark:text-slate-200 text-xs transition">
                ...
                <td class="p-3 font-semibold text-amber-600 dark:text-amber-400">
                  ...
                  <div v-if="item.pending_rate" class="text-[9px] text-amber-500 font-bold mt-0.5" title="Pengajuan rate baru">
                ...
                <td class="p-3">
                  <div class="flex gap-1.5 items-center flex-wrap">
                    ...
                    <button v-if="item.pending_rate" @click="approveRate(item)" class="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-bold transition">✓ Setujui Rate</button>
      ...
      <div class="card w-full max-w-lg p-6 animate-pop dark:bg-slate-900 dark:border-slate-800 space-y-4">
        ...
        <form @submit.prevent class="space-y-4 pt-2 border-t border-[#E8D5C8]/40">
          ...
          <div class="flex gap-2 justify-end pt-2">
            ...
            <button type="button" @click="submitDecision('rejected')" class="px-5 py-2.5 bg-rose-50 text-rose-600 hover:bg-rose-100 border border-rose-200 rounded-xl text-xs font-semibold transition">Tolak (Reject)</button>
            <button type="button" @click="submitDecision('approved')" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold transition shadow-md">Setujui (Approve)</button>
          </div>
        </form>
      </div>
    </div>
