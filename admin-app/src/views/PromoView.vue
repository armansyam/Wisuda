  ...
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Kelola kode diskon untuk klien dan program afiliasi.</p>
      </div>
    ...
    <div class="border-b border-gray-200">
      <nav class="-mb-px flex space-x-8">
        <button 
          ...
          :class="[activeTab === 'promo' ? 'border-[#0f766e] text-[#0f766e]' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm']"
        >
          Kode Promo (Reguler)
        </button>
        <button 
          @click="activeTab = 'partner'"
          :class="[activeTab === 'partner' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm']"
        >
          Partner & Afiliasi
        </button>
      </nav>
          ...
          <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th class="px-6 py-4 font-semibold text-gray-900">Jumlah Diskon</th>
                <th class="px-6 py-4 font-semibold text-gray-900">Quota Limit</th>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-if="promos.length === 0">
                <td colspan="5" class="px-6 py-8 text-center text-gray-500">Belum ada kode promo.</td>
              </tr>
      ...
      <div v-if="!loading" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="px-6 py-4 font-semibold text-gray-900">Nama Partner</th>
                <th class="px-6 py-4 font-semibold text-gray-900">Profesi</th>
                <th class="px-6 py-4 font-semibold text-gray-900">Kode</th>
                <th class="px-6 py-4 font-semibold text-gray-900">Diskon Klien</th>
                <th class="px-6 py-4 font-semibold text-gray-900">Fee Komisi</th>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-if="partners.length === 0">
                <td colspan="8" class="px-6 py-8 text-center text-gray-500">Belum ada partner/afiliasi.</td>
              </tr>
              <tr v-for="partner in partners" :key="partner.id" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-4 text-gray-500 text-xs">{{ partner.profession }}</td>
                <td class="px-6 py-4">
                  <span class="inline-block px-2.5 py-1 bg-gray-100 text-gray-800 font-mono font-bold text-xs rounded border border-gray-200">{{ partner.code }}</span>
                </td>
                <td class="px-6 py-4 font-medium text-emerald-600">
                ...
                <td class="px-6 py-4 text-gray-900 font-semibold">{{ partner.usage_count }}</td>
                <td class="px-6 py-4">
                  <button @click="toggleActivePartner(partner)" 
                    :class="partner.active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'"
                    class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-colors">
    ...
    <!-- Modal Form Promo -->
    <div v-if="showPromoModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden" @click.stop>
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 class="font-bold text-gray-900 text-lg">Buat Kode Promo Reguler</h3>
          <button @click="showPromoModal = false" class="text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <form @submit.prevent="savePromo" class="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" v-model="formPromo.code" required class="w-full text-sm rounded-lg border-gray-300 focus:border-[#0f766e] focus:ring focus:ring-[#0f766e]/20 uppercase font-mono" placeholder="MISAL: SINTA100">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="formPromo.discount_type" class="w-full text-sm rounded-lg border-gray-300 focus:border-[#0f766e] focus:ring focus:ring-[#0f766e]/20">
            ...
            <div>
              ...
              <input type="number" v-model="formPromo.discount_value" required min="1" class="w-full text-sm rounded-lg border-gray-300 focus:border-[#0f766e] focus:ring focus:ring-[#0f766e]/20" placeholder="50000">
            </div>
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">Batas Kuota Pemakaian</label>
            <input type="number" v-model="formPromo.quota" min="1" class="w-full text-sm rounded-lg border-gray-300 focus:border-[#0f766e] focus:ring focus:ring-[#0f766e]/20" placeholder="Kosongkan jika unlimited">
          </div>

          <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
            <button type="button" @click="showPromoModal = false" class="px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Batal</button>
    ...
    <!-- Modal Form Partner -->
    <div v-if="showPartnerModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 sticky top-0 z-10">
          <h3 class="font-bold text-gray-900 text-lg">Buat Partner / Afiliasi Baru</h3>
          <button @click="showPartnerModal = false" class="text-gray-400 hover:text-gray-600 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
        ...
        <form @submit.prevent="savePartner" class="p-6 space-y-4">
          ...
          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">Nama Partner <span class="text-red-500">*</span></label>
            <input type="text" v-model="formPartner.name" required class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-600 focus:ring focus:ring-emerald-600/20" placeholder="Cth: Budi Santoso">
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">Profesi / Tipe Partner <span class="text-red-500">*</span></label>
            <select v-model="formPartner.profession" required class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-600 focus:ring focus:ring-emerald-600/20">
          ...
          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">Kode Referal Unik <span class="text-red-500">*</span></label>
            <input type="text" v-model="formPartner.code" required class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-600 focus:ring focus:ring-emerald-600/20 uppercase font-mono" placeholder="MISAL: BUDI123">
            <p class="text-[10px] text-gray-500 mt-1">Klien memasukkan kode ini saat booking.</p>
          </div>

          <div class="p-3 bg-gray-50 rounded-lg border border-gray-100">
            <h4 class="text-xs font-semibold text-gray-800 mb-3">Pengaturan Diskon Klien</h4>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-bold text-gray-700 mb-1">Tipe Diskon</label>
                <select v-model="formPartner.discount_type" class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-600 focus:ring focus:ring-emerald-600/20">
                  <option value="nominal">Nominal (Rp)</option>
                  <option value="percent">Persen (%)</option>
                </select>
              </div>
              <div>
                ...
                <input type="number" v-model="formPartner.discount_value" required min="0" class="w-full text-sm rounded-lg border-gray-300 focus:border-emerald-600 focus:ring focus:ring-emerald-600/20" placeholder="0">
              </div>
            </div>
          </div>

          <div class="p-3 bg-amber-50 rounded-lg border border-amber-100">
            <h4 class="text-xs font-semibold text-amber-800 mb-3">Pengaturan Fee / Komisi Partner</h4>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-bold text-amber-700 mb-1">Tipe Komisi</label>
              ...
              <div>
                <label class="block text-xs font-bold text-amber-700 mb-1">Nilai Komisi</label>
          ...
          <div class="pt-4 border-t border-gray-100 flex justify-end gap-3 sticky bottom-0 bg-white">
            <button type="button" @click="showPartnerModal = false" class="px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Batal</button>
