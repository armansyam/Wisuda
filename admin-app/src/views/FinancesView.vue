<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-5">
      <h2 class="text-xl font-bold text-[#2D1B14] tracking-tight">Keuangan & Payroll</h2>
    </div>
    ...
    <div class="flex gap-2 mb-4">
      <button @click="activeTab = 'dps'" class="px-4 py-2 rounded-xl text-xs font-semibold transition"
        :class="activeTab === 'dps' ? 'bg-[#2D1B14] text-[#D4AF37]' : 'bg-white text-[#8A7A72] border border-[#E8D5C8]/80 hover:bg-[#FFF8F3]'">
        💵 DP Client
      </button>
      <button @click="activeTab = 'payouts'" class="px-4 py-2 rounded-xl text-xs font-semibold transition"
        :class="activeTab === 'payouts' ? 'bg-[#2D1B14] text-[#D4AF37]' : 'bg-white text-[#8A7A72] border border-[#E8D5C8]/80 hover:bg-[#FFF8F3]'">
        🙋‍♂️ Payroll Freelance
      </button>
    </div>

    <!-- TAB 1: DP Client -->
    <div v-show="activeTab === 'dps'" class="card overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-if="dps.length === 0">
            <td class="p-3 text-[#C4B0A5] text-center" colspan="4">Belum ada data DP</td>
          </tr>
        </tbody>
      </table>
    </div>

    <!-- TAB 2: Payroll Freelance -->
    <div v-show="activeTab === 'payouts'" class="space-y-4">
      <!-- Filter Payout Status -->
      <div class="flex items-center gap-2 mb-2 bg-[#FFF8F3] p-3 rounded-xl border border-[#E8D5C8]/40">
        <span class="text-xs text-[#8A7A72] font-medium">Filter Status Payout:</span>
        <select v-model="filterStatus" @change="loadPayouts" class="text-xs px-2.5 py-1.5 border border-[#E8D5C8] rounded-lg bg-white text-[#2D1B14] focus:outline-none">
      ...
      <div v-if="filterStatus === 'pending'" class="space-y-4">
        <div v-for="g in groupedPendingPayouts" :key="g.fg_id" class="card p-5 border border-[#E8D5C8]/60 hover:shadow-md transition">
          <!-- Freelancer Profile Header -->
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-[#E8D5C8]/60 pb-3 mb-3 gap-3">
            <div>
              <h3 class="font-bold text-base text-[#2D1B14]">{{ g.fg_name }}</h3>
              <p class="text-xs text-[#8A7A72]">{{ g.fg_phone }}</p>
            </div>
            
            <!-- Bank Details -->
            <div class="bg-[#FAF6F0] p-2.5 rounded-xl border border-[#E8D5C8]/40 text-xs text-[#8A7A72]">
              <div v-if="g.bank_account?.bank">
                <span class="font-semibold text-[#2D1B14]">{{ g.bank_account.bank }}</span> - {{ g.bank_account.norek }}
          ...
          <div class="overflow-x-auto text-xs mb-4">
            <table class="w-full text-left">
              <thead>
                <tr class="text-[#8A7A72] border-b border-[#E8D5C8]/40">
                  <th class="p-2 w-10">
                    <input type="checkbox" 
              ...
              <tbody>
                <tr v-for="a in g.assignments" :key="a.assignment_id" class="border-b border-[#E8D5C8]/20 hover:bg-[#FFF8F3]/50">
                  <td class="p-2">
                    <input type="checkbox" 
                  ...
                  <td class="p-2">
                    <div class="font-medium text-[#2D1B14]">{{ a.client_name }}</div>
                    <div class="text-[10px] text-[#8A7A72]">{{ formatDate(a.graduation_date) }}</div>
                  </td>
                  <td class="p-2 text-[#8A7A72]">{{ a.location || '-' }}</td>
                  <td class="p-2 text-right font-semibold text-[#2D1B14]">Rp {{ (a.total_payout || 0).toLocaleString('id-ID') }}</td>
                  <td class="p-2 text-center">
                    <span class="status-chip bg-blue-50 text-blue-600 border border-blue-200">
          ...
          <div class="flex justify-between items-center bg-[#FFF8F3]/60 p-3 rounded-xl border border-[#E8D5C8]/30">
            <div class="text-xs text-[#8A7A72]">
              Terpilih: <span class="font-bold text-[#2D1B14]">{{ selectedAssignments[g.fg_id]?.length || 0 }}</span> dari {{ g.assignments.length }} project
        ...
        <!-- Empty State Grouped -->
        <div v-if="groupedPendingPayouts.length === 0" class="text-center py-16 text-[#C4B0A5] card">
      ...
      <div v-else class="card overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="p in payouts" :key="p.id" class="border-b border-[#E8D5C8]/60 hover:bg-[#FFF8F3] text-[#2D1B14]">
              <td class="p-3">
                <div class="font-medium">{{ p.fg_name }}</div>
                <div class="text-[10px] text-[#8A7A72]">{{ p.fg_phone }}</div>
              </td>
              <td class="p-3">
                <div class="font-medium text-xs whitespace-pre-line">{{ p.client_name }}</div>
                <div v-if="p.status !== 'paid'" class="text-[10px] text-[#8A7A72]">{{ formatDate(p.graduation_date) }}</div>
              </td>
              <td class="p-3 font-semibold text-[#D94A3D]">
                Rp {{ (p.total_payout || 0).toLocaleString('id-ID') }}
              </td>
              <td class="p-3 text-xs text-[#8A7A72]">
                <div v-if="p.bank_account?.bank">
                  <span class="font-semibold text-[#2D1B14]">{{ p.bank_account.bank }}</span> - {{ p.bank_account.norek }}
            ...
            <tr v-if="payouts.length === 0">
              <td class="p-3 text-[#C4B0A5] text-center" colspan="6">Belum ada data Payout</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showPayModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(45,27,20,0.6); backdrop-filter: blur(6px);" @click.self="showPayModal = false">
      <div class="card w-full max-w-sm p-6 animate-pop">
        <h3 class="font-bold text-lg text-[#2D1B14] mb-1">Konfirmasi Pembayaran</h3>
        <p class="text-xs text-[#8A7A72] mb-4">Kirim fee ke fotografer <span class="font-bold text-[#2D1B14]">{{ payItem?.fg_name }}</span></p>
        
        <!-- Bank Target Info -->
        <div class="bg-[#FFF8F3] border border-[#E8D5C8]/80 p-3 rounded-xl mb-4 text-xs">
          <p class="text-[10px] text-[#8A7A72] uppercase font-bold tracking-wider">Rekening Tujuan</p>
          <div v-if="payItem?.bank_account?.bank" class="mt-1">
            <div class="font-semibold text-[#2D1B14]">{{ payItem.bank_account.bank }} - {{ payItem.bank_account.norek }}</div>
            <div>A/N: {{ payItem.bank_account.atas_nama }}</div>
        ...
        <form @submit.prevent="submitPayment" class="space-y-4">
          <div class="flex gap-2 justify-end mt-5">
            <button type="button" @click="showPayModal = false" class="px-4 py-2 text-xs font-semibold text-[#8A7A72] hover:text-[#2D1B14] transition">
              Batal
            </button>
    ...
    <div v-if="showInvoiceModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" style="background: rgba(45,27,20,0.6); backdrop-filter: blur(6px);" @click.self="showInvoiceModal = false">
      <div class="card w-full max-w-lg p-8 animate-pop relative my-8" id="printable-payout-invoice">
        <button @click="showInvoiceModal = false" class="absolute top-4 right-4 text-[#8A7A72] hover:text-[#2D1B14] text-xl font-bold no-print">×</button>
        
        <!-- Invoice Header -->
        <div class="flex justify-between items-start border-b border-[#E8D5C8] pb-5 mb-5">
          <div>
            <h1 class="text-lg font-bold text-[#2D1B14] uppercase tracking-wider">Sorehari Wisuda</h1>
            <p class="text-[10px] text-[#8A7A72] mt-0.5">Photography Agency & Graduation Services</p>
          </div>
          <div class="text-right">
            <h2 class="text-xs font-bold text-[#D94A3D] uppercase tracking-wide">Payout Invoice</h2>
            <p class="text-[10px] text-[#8A7A72] mt-0.5">Invoice: #PAY-{{ invoiceItem?.id }}</p>
            <p class="text-[10px] text-[#8A7A72] mt-0.5">Tanggal: {{ formatDate(invoiceItem?.created_at || new Date().toISOString()) }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 text-xs mb-6">
          <div>
            <p class="text-[9px] uppercase tracking-wider text-[#C4B0A5] font-bold mb-1">Penerima (Freelancer)</p>
            <p class="font-bold text-[#2D1B14]">{{ invoiceItem?.fg_name }}</p>
            <p class="text-[#8A7A72] mt-0.5">{{ invoiceItem?.fg_phone }}</p>
            <div v-if="invoiceItem?.bank_account?.bank" class="mt-1 bg-[#FAF6F0] p-2 rounded-lg border border-[#E8E4D8]/60 text-[#2D1B14]">
              <span class="font-semibold text-[#2D1B14]">{{ invoiceItem.bank_account.bank }}</span><br>
              Norek: {{ invoiceItem.bank_account.norek }}<br>
              A/N: {{ invoiceItem.bank_account.atas_nama }}
            </div>
          </div>
          <div class="text-right">
            <p class="text-[9px] uppercase tracking-wider text-[#C4B0A5] font-bold mb-1">Status Pembayaran</p>
            ...
            <p class="text-[10px] text-[#8A7A72] mt-2" v-if="invoiceItem?.status === 'paid'">Ref No: {{ invoiceItem?.transfer_ref }}</p>
            <p class="text-[10px] text-[#8A7A72]" v-if="invoiceItem?.status === 'paid'">Dibayar: {{ invoiceItem?.paid_at ? formatDate(invoiceItem.paid_at) : '-' }}</p>
          </div>
        </div>

        <!-- Invoice Items Table -->
        <div class="border border-[#E8D5C8] rounded-xl overflow-hidden mb-6 text-xs">
          <table class="w-full">
            <thead>
              <tr class="bg-[#FFF8F3] text-[#8A7A72] text-left border-b border-[#E8D5C8]">
                <th class="p-2.5 font-medium">Client & Jadwal</th>
                <th class="p-2.5 font-medium text-right">Fee Payout</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in invoiceItem?.items" :key="item.payout_id" class="border-b border-[#E8D5C8]/60 text-[#2D1B14]">
                <td class="p-2.5">
                  <div class="font-bold text-[#2D1B14]">{{ item.client_name }}</div>
                  <div class="text-[10px] text-[#8A7A72] mt-0.5">{{ formatDate(item.graduation_date) }}</div>
                </td>
                <td class="p-2.5 text-right font-semibold text-gray-800">Rp {{ (item.fg_fee || item.total_payout || 0).toLocaleString('id-ID') }}</td>
              </tr>
              <tr v-if="invoiceItem?.bonus > 0" class="border-b border-[#E8D5C8]/60 text-[#2D1B14]">
                <td class="p-2.5">
                  Bonus / Kompensasi Tambahan
                </td>
                <td class="p-2.5 text-right font-medium text-[#059669]">+ Rp {{ (invoiceItem.bonus).toLocaleString('id-ID') }}</td>
              </tr>
              <tr v-if="invoiceItem?.deduction > 0" class="border-b border-[#E8D5C8]/60 text-[#2D1B14]">
                <td class="p-2.5">
                  Potongan / Penalty
                </td>
            ...
            <tfoot>
              <tr class="bg-[#FFF8F3] font-bold text-[#2D1B14]">
...
const payForm = ref({ transfer_ref: '', slip_url: '' })
...
// Complete Payment handlers
function openPayModal(payout) {
  payItem.value = {
    fg_name: payout.fg_name,
    fg_phone: payout.fg_phone,
    bank_account: payout.bank_account,
    assignment_ids: [payout.assignment_id || payout.id],
    total_payout: payout.total_payout
  }
  payForm.value = { transfer_ref: '', slip_url: '' }
...
async function submitPayment() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Auto open the WA link returned by the server
      if (d.wa_link) {
...
// Pre-payment validation message
function getWaValidateLink(p) {
  const bank = p.bank_account?.bank || '-'
  const norek = p.bank_account?.norek || '-'
  const atas_nama = p.bank_account?.atas_nama || '-'
  const text = `Halo ${p.fg_name}, mohon konfirmasi rincian fee berikut sebelum kami transfer:

Client: ${p.client_name}
Tanggal: ${formatDate(p.graduation_date)}
Fee: Rp ${(p.total_payout || 0).toLocaleString('id-ID')}
...
Jika data di atas sudah sesuai, harap membalas pesan ini agar proses transfer dapat segera dilanjutkan. Terima kasih!`.trim()
