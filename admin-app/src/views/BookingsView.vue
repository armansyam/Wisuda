        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="(item, idx) in sortedBookings" :key="item.id"
              ...
              <!-- Kolom Direct Drive Upload Bertahap -->
              <td class="p-3" @click.stop>
                <div v-if="getDriveUploadButton(item)" class="flex items-center">
                  <a :href="getDriveUploadButton(item).url" target="_blank"
                     class="px-2 py-1 rounded text-[9px] font-bold text-white transition shadow-sm flex items-center gap-1 cursor-pointer"
                     :class="getDriveUploadButton(item).bgClass"
                     :title="getDriveUploadButton(item).title">
      ...
      <div class="card w-full max-w-md p-6 animate-pop dark:bg-slate-900 dark:border-slate-800" @click.stop>
        ...
        <div v-if="!driveForm.drive_parent_url" class="mb-3.5 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-between">
          <div>
            ...
            <p class="text-[9px] text-amber-600/80 dark:text-amber-400/80">Buat struktur folder induk & subfolder otomatis via bot.</p>
          </div>
...
async function autoGenerateDriveFolder(item) {
  ...
  if (!confirm(`Generate otomatis folder Google Drive untuk client ${item.client_name}?`)) return
