(function() {
  ...
  popup.innerHTML = `
    <div style="display: flex; justify-content: space-between; align-items: center; border: none; background: transparent; padding: 0;">
      <span style="font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #64748b; font-weight: 700; text-align: left;">AmsDev Credit</span>
    ...
    fetch('/api/public/version')
      ...
      .then(d => {
        if (d && (d.release || d.version)) {
          ...
          if (vTag) {
            const cleanVer = d.version ? (d.version.startsWith('v') ? d.version : 'v' + d.version) : (d.release ? d.release.split(' ')[0] : 'v1.3.1');
            vTag.textContent = cleanVer;
          }
